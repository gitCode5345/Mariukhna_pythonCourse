...
def send_money(sender_account_id: int, receiver_account_id: int, amount: float, transaction_date=None, cursor=None):
    ...
    try:
        sender_response = cursor.execute('''SELECT amount, currency, bank_id
                                            FROM Account
                                            WHERE id=?''', (sender_account_id,)).fetchone()
        ...
        receiver_response = cursor.execute('''SELECT amount, currency, bank_id
                                              FROM Account
                                              WHERE id=?''', (receiver_account_id,)).fetchone()
        ...
        cursor.execute('''UPDATE Account
                          SET amount=?
                          WHERE id=?''', (float(sender_amount), int(sender_account_id)))
        cursor.execute('''UPDATE Account
                          SET amount=?
                          WHERE id=?''', (float(receiver_amount), int(receiver_account_id)))

        sender_bank_name = cursor.execute('''SELECT name
                          FROM Bank
                          WHERE id=?''', (sender_bank_id,)).fetchone()
        receiver_bank_name = cursor.execute('''SELECT name
                          FROM Bank
                          WHERE id=?''', (receiver_bank_id,)).fetchone()
